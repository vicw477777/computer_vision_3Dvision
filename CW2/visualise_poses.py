...
def main(
    scene_path: Path = Path(__file__).parent / "results/colmap/static_scene",
    downsample_factor: int = 4,
    reorient_scene: bool = True,
) -> None:
    """Visualize COLMAP sparse reconstruction (cameras + point cloud) in viser.

    Args:
        scene_path: Path to the scene directory (contains colmap/sparse/0 and images).
        downsample_factor: Downsample factor for images.
        reorient_scene: Whether to reorient the scene so that average camera up direction is pointing up.

    Example:
        python viser_colmap_visualize.py --scene-path /path/to/computed/pose/folder
    """
    scene_path = scene_path.resolve()
    colmap_path = scene_path / "colmap/sparse/0"
